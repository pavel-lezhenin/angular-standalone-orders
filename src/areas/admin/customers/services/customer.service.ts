...
export class CustomerService {
  ...
  async loadUsers(
    ...
    return firstValueFrom(
      this.http.get<PaginatedResponse<UserDTO>>(this.apiUrl, { params: httpParams })
    );
  }
   ...
   * Create new user
   */
  async createUser(formValue: CustomerFormData): Promise<UserDTO> {
    const dto: CreateUserDto = {
      ...this.mapFormToDto(formValue),
      password: formValue.password,
    };

    return firstValueFrom(this.http.post<UserDTO>(this.apiUrl, dto));
  }

  /**
   * Update existing user
   */
  async updateUser(userId: string, formValue: CustomerFormData): Promise<UserDTO> {
    const dto = this.mapFormToDto(formValue) as UpdateUserDto;

    return firstValueFrom(this.http.put<UserDTO>(`${this.apiUrl}/${userId}`, dto));
  }
}
